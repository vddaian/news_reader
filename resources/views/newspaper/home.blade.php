...
@section('title', 'Home')
@section('content')
    <div class="container-xxl mt-2 mb-5 p-0 row" style="height: 35%">
        ...
        <section class="col-6 my-3 h-100 d-flex align-items-center overflow-auto overflow-x-hidden flex-column bg-light rounded">
            @if (isset($data['newspapers']))
                @foreach ($data['newspapers'] as $newspaper)
                    <div class="w-100 mx-2 mt-2 p-3 bg-white d-flex align-items-center justify-content-between rounded">
                        <h5 class="w-50">{{ $newspaper[0]->title }}</h5>
                        <div class="d-flex">
                            <form class="px-1" action="{{ route('newsp.show', $newspaper[0]->id) }}" method="get">
                                <button class="btn btn-success" type="submit">View</button>
                            </form>
                            <form class='px-1' action="{{route('newsp.delete', $newspaper[0]->id)}}" method="post">
                                @method('delete')
                                @csrf
